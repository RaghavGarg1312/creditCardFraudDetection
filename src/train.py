...
def train_and_evaluate_model(model_name, model, X_train, y_train, X_test, y_test, cv_method):
    ...
    with mlflow.start_run(run_name=model_name):
        ...
        # Use XGBoost's native save_model for XGBoost, joblib for others
        if model_name == "XGBoost":
            model_path = f"models/{model_name}_model.json"
            model.save_model(model_path)
            # Also save with joblib for backward compatibility with app.py
            joblib.dump(model, f"models/{model_name}_model.pkl")
            print(f"{model_name} model saved to 'models/{model_name}_model.json' and 'models/{model_name}_model.pkl' with avg accuracy: {avg_score:.4f}")
        else:
            joblib.dump(model, f"models/{model_name}_model.pkl")
